...
class database(object):
	...
	def database_init(self):
		...
		self.cur.execute('CREATE TABLE IF NOT EXISTS records ( steamID INT(15), rec_type VARCHAR(30), matchID INT(15), team VARCHAR(30), hero_id INT(15), victory INT(15), no_tower INT(15), gpm INT(15), xpm INT(15), max_gold INT(15), max_xp INT(15), lh INT(15), denies INT(15), kills INT(15), assists INT(15), deaths INT(15), kda INT(15), hDMG INT(15), tDMG INT(15), hHEAL INT(15), l25_time INT(15), fblood_time INT(15), total_time INT(15));')
		...
		self.cur.execute('CREATE TABLE IF NOT EXISTS meta (steamID INT, last_checked INT, wins INT, games INT, IRCname VARCHAR(50));')
	...
	def database_print(self):
		...
		for name in databases:
			self.cur.execute('SELECT * FROM %s', [name[0]])
	...
	def write_match(self, steamID, matchID):
		...
		self.cur.execute('SELECT name FROM sqlite_master WHERE type="table" AND name=%s;', [steamID])
		already_exists = self.cur.fetchone()

		if not already_exists:
			self.cur.execute('CREATE TABLE %s (matchID INT);', [steamID])	
			logger.info('Created db-table for SteamID: {}'.format(steamID))


		self.cur.execute('SELECT matchID FROM %s WHERE matchID = %s', [steamID, matchID])
		already_exists = self.cur.fetchone()
		if not already_exists:
			self.cur.execute('INSERT INTO %s VALUES (%s);', [steamID, matchID])
	...
	def return_todolist(self, steamID):
		self.cur.execute('SELECT MatchID FROM todolist WHERE steamID = %s ORDER BY matchID ASC;', [steamID])
	...
	def todolist_match(self, steamID, matchID):
		...
		self.cur.execute('INSERT INTO todolist VALUES (%s, %s);', [steamID, matchID])
	...
	def delete_entry_in_todolist(self, steamID, matchID):
		self.cur.execute('DELETE FROM todolist WHERE steamID = %s AND MatchID = %s', [steamID, matchID])
		self.cur.execute('SELECT * FROM todolist WHERE steamID = %s AND MatchID = %s', [steamID, matchID])
	...
	def store_records(self, steamID, matchID, team, hero_id, victory, abandon, no_tower, gpm, xpm, max_gold, max_xp, lh,
		...
		self.cur.execute('SELECT * FROM records where steamID = %s', [steamID])
		...
		if not init: # hopefully this will work with an empty database.
			...
			for record_types in record_list:
				...
				self.cur.execute('INSERT INTO records VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);',
			...
			self.cur.execute('UPDATE records SET deaths = 0 WHERE steamID = %s and rec_type = "no_deaths"', [steamID,])



		logger.info('Found new record of type: {0}'.format(rec_type))
		if not rec_type == 'no_tower' and not rec_type == 'no_deaths':
			self.cur.execute('UPDATE records SET steamID=%s, rec_type=%s, matchID=%s, team=%s, hero_id=%s, victory=%s, no_tower=%s, gpm=%s, xpm=%s, max_gold=%s, max_xp=%s, lh=%s, denies=%s, kills=%s, assists=%s, deaths=%s, kda=%s, hDMG=%s, tDMG=%s, hHEAL=%s, l25_time=%s, fblood_time=%s, total_time=%s WHERE rec_type = %s AND steamID = %s;',
			...
			self.cur.execute('UPDATE records SET steamID=%s, matchID=%s, team=%s, hero_id=%s, victory=%s, no_tower=(no_tower+1), gpm=%s, xpm=%s, max_gold=%s, max_xp=%s, lh=%s, denies=%s, kills=%s, assists=%s, deaths=%s, kda=%s, hDMG=%s, tDMG=%s, hHEAL=%s, l25_time=%s, fblood_time=%s, total_time=%s WHERE rec_type = %s AND steamID = %s;',
			...
			self.cur.execute('UPDATE records SET steamID=%s, matchID=%s, team=%s, hero_id=%s, victory=%s, no_tower=%s, gpm=%s, xpm=%s, max_gold=%s, max_xp=%s, lh=%s, denies=%s, kills=%s, assists=%s, deaths=(deaths+1), kda=%s, hDMG=%s, tDMG=%s, hHEAL=%s, l25_time=%s, fblood_time=%s, total_time=%s WHERE rec_type = %s AND steamID = %s;',
	...
	def store_meta(self, steamID, last_checked=None, victory=None, IRCname=None):
		init = None
		while not init:
			self.cur.execute('SELECT * FROM meta WHERE steamID = %s', [steamID])
			init = self.cur.fetchone()
			if not init:
				# here should be at least (SteamID, 0, 0, 0, 0)
				self.cur.execute('INSERT INTO meta VALUES (%s, 0, 0, 0, 0)', [steamID])
				logger.info('Generated new META entry for SteamID {0}'.format(steamID))

		if last_checked > init[1]:
			self.cur.execute('UPDATE meta SET last_checked = %s WHERE steamID = %s', [last_checked, steamID])
		if victory:
			self.cur.execute('UPDATE meta SET wins = (wins + 1), games = (games + 1) WHERE steamID = %s', [steamID])
		if not victory:
			self.cur.execute('UPDATE meta SET games = (games + 1) WHERE steamID = %s', [steamID])
		if IRCname:
			self.cur.execute('UPDATE meta SET IRCname = %s WHERE steamID = %s', [IRCname, steamID])
			logger.info('Updated the IRCname for SteamID{0} to {1}.'.format(steamID, IRCname))


	def get_last_checked(self, steamID):
		self.cur.execute('SELECT last_checked FROM meta WHERE steamID = %s', [steamID])
	...
	def ircname_to_steamid(self, IRCname):
		...
		self.cur.execute('SELECT steamID FROM meta WHERE IRCname = %s', [IRCname])
		data = self.cur.fetchone()
		return data[0]
	...
	def return_record(self, steamID, record):
		...
		self.cur.execute('SELECT * FROM records WHERE steamID = %s AND rec_type=%s', [steamID, record])
		data = self.cur.fetchone()
